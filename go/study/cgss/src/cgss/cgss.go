...
func Login(args []string) int {
	...
	level1 , err := strconv.Atoi(args[2])
	if err != nil {
		fmt.Println("Invalid paramtere:<level> should be an integer")
		return 0 
	}
	exp , err := strconv.Atoi(args[3])
	if err != nil {
		fmt.Println("Invalid paramtere:<exp> should be an integer")
		return 0 
	}
...
func main(){
	fmt.Println("Casual game Serer Solution")

	startCenterService()

	Help(nil)
	r := bufio.NewReader(os.Stdin)
	hanlders := GetCommandHandlers()
	...
	for {
		...
		if hanlder , ok := hanlders[tokens[0]] ; ok {
			ret  := hanlder(tokens)
			if ret != 0 {
				break
			}
