...
import uk.ac.gla.terrier.matching.tsms.TermInFieldModifier;
...
public class Manager implements TermPipeline, TermPipelineAccessor
{
	...
	protected void load_controls_default()
	{
		...
		String def_c = null;
		/* add the c property as the default c value in the search request */
		if ((def_c = ApplicationSetup.getProperty("c", null)) != null)
			Default_Controls.put("c", def_c);
	...
	public void runMatching(SearchRequest srq)
	{
		...
		if ( (! rq.isEmpty()) || MATCH_EMPTY_QUERY )
		{
			...
			//if(logger.isDebugEnabled()){
			//	logger.debug("c_set="+rq.getControl("c_set"));
			//}
