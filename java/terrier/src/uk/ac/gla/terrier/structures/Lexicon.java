...
public class Lexicon implements Iterable<String>, Closeable{
	...
	public Lexicon(String lexiconName) {
		try {
			lexiconFile = new RandomAccessFile(this.lexiconFileName = lexiconName, "r");
			idToOffsetFile = new RandomAccessFile(lexiconName.substring(0,lexiconName.lastIndexOf(".")).concat(ApplicationSetup.LEXICON_INDEX_SUFFIX),"r");
	...
	*/
	...
	public boolean findTerm(int _termId) {
		try {
			...
			return seekEntry((int)idToOffsetFile.readLong()/lexiconEntryLength);
		} catch(IOException ioe) {
	...
	public boolean findTerm(String _term) {
		...
		try {
			while (high-low>1) {
				...
				lexiconFile.read(buffer, 0, lexiconEntryLength);
		...
		try {
			if (i >= numberOfLexiconEntries || i < 0)
				...
				if (i == 0) {
					...
					lexiconFile.seek((i-1) * lexiconEntryLength + ApplicationSetup.STRING_BYTE_LENGTH + 12);
					lexiconFile.read(buffer, 0, lexiconEntryLength + 9);
					dataInput.reset();
	...
	public static int numberOfEntries(File f) {
		return (int)(f.length())/lexiconEntryLength;
	}
		...
		try {
			while (high-low>1) {
				...
				if (i==0) {
					...
					lexiconFile.seek(i * lexiconEntryLength-9);
					lexiconFile.read(buffer, 0, lexiconEntryLength+9);
					term = new String(buffer,9,ApplicationSetup.STRING_BYTE_LENGTH).trim();
				}
			...
			if (high == 0) {
				...
				lexiconFile.seek(high * lexiconEntryLength-9);
				lexiconFile.read(buffer, 0, lexiconEntryLength+9);
				term = new String(buffer,9,ApplicationSetup.STRING_BYTE_LENGTH).trim();				
			}
