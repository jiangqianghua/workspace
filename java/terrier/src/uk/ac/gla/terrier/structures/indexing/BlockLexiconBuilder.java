...
public class BlockLexiconBuilder extends LexiconBuilder
{
	...
	/*public void merge(LinkedList filesToMerge) throws IOException {
		...
		if (StartFileCount == 1) {
			...
			logger.info("begin merging "+ StartFileCount +" files...");
			...
			while (filesToMerge.size() > 1) {
				...
				BlockLexiconInputStream lis1 = new BlockLexiconInputStream(fileToMerge1);
				BlockLexiconInputStream lis2 = new BlockLexiconInputStream(fileToMerge2);
				BlockLexiconOutputStream los = new BlockLexiconOutputStream(newMergedFile);
	
				logger.info("merging " + fileToMerge1 + " with " + fileToMerge2 + " to " + newMergedFile);
			...
			logger.info("end of merging...("+((endTime-startTime)/1000.0D)+" seconds)");
		}
	}*/
