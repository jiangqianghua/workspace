...
import uk.ac.gla.terrier.structures.indexing.singlepass.FieldPosting;
...
public class Inverted2DirectIndexBuilder {
	...
	public void createDirectIndex()
	{
		...
		try{
			...
			final BitOutputStream bos = new BitOutputStream(index.getPath() + ApplicationSetup.FILE_SEPARATOR + index.getPrefix() + ApplicationSetup.DF_SUFFIX);
			do//for each pass of the inverted file
			{
				...
				{	
					...
					if (p.getDocF() > 0)
					{
						...
						final SimplePostingInRun pir = getPostingReader();
						pir.setDf(p.getDocF());
						pir.setTF(p.getTF());
			...
			dis.close();
			dios.close();
			new File(index.getPath() + ApplicationSetup.FILE_SEPARATOR + index.getPrefix() + ApplicationSetup.DF_SUFFIX + ".offsets").delete();
			new File(index.getPath() + ApplicationSetup.FILE_SEPARATOR + index.getPrefix() + ApplicationSetup.DOC_INDEX_SUFFIX).delete();
			new File(
					index.getPath() + ApplicationSetup.FILE_SEPARATOR + index.getPrefix() + ApplicationSetup.DOC_INDEX_SUFFIX + ".withoffsets")
				.renameTo(new File(index.getPath() + ApplicationSetup.FILE_SEPARATOR + index.getPrefix() + ApplicationSetup.DOC_INDEX_SUFFIX));
	...
	protected SimplePostingInRun getPostingReader()
	{
		if (saveTagInformation)
		{
			return new FieldPostingInRun(0);
		}
		return new SimplePostingInRun(0);
	}
