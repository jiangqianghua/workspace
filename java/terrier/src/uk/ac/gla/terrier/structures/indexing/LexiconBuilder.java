...
public class LexiconBuilder
{
	...
	protected void writeTemporaryLexicon()
	{
		...
		try{
			...
			if (!(new File(TemporaryLexiconDirectory + TempLexDirCount)).exists()) {
				...
				tmpDir.deleteOnExit();//it's fine to mark the temporary *directory* for deletion
				tmpDir.mkdir();
			}
	...
	public void addDocumentTerms(FieldDocumentTreeNode[] terms)
	{
		...
		for (FieldDocumentTreeNode t: terms)
		{
			TempLex.insert(t.term, t.frequency/*, t.termCode*/);
		}
		DocCount++;
	...
	public void merge(LinkedList<String> filesToMerge) throws IOException {
		...
		if (StartFileCount == 1) {
			(new File(filesToMerge.removeFirst())).
				renameTo((new File(indexPath +
		...
		//more than 1 lexicon to merge, but configured only to merge 2 at a time	
			...
			while (filesToMerge.size() > 1) {
				...
				(new File(fileToMerge1)).delete();
				(new File(fileToMerge2)).delete();
		...
		else if (mergeNMaxLexicon > 0 && StartFileCount > mergeNMaxLexicon)
		{
			...
			while (filesToMerge.size() > 1)
			{
				...
				for(int i=0;i<numLexicons;i++)
				{
					...
					//logger.debug(i+" "+inputLexiconFileNames[i] +" size="+new File(inputLexiconFileNames[i]).length());
				}
				...
				if (filesToMerge.size() == 0)
					...
					newMergedFile =
						(new File(inputLexiconFileNames[0])).getParent()
							+ ApplicationSetup.FILE_SEPARATOR
							+ ApplicationSetup.MERGE_PREFIX
							+ String.valueOf(progressiveNumber++)
							+ ApplicationSetup.LEXICONSUFFIX;
				...
				for(int i=0;i<numLexicons;i++)
				{
					new File(inputLexiconFileNames[i]).delete();
				}
				filesToMerge.addLast(newMergedFile);
			}
			long endTime = System.currentTimeMillis();
			...
			{
				String oldName =  filesToMerge.removeFirst();
				new File(oldName).renameTo(new File(indexPath +
					ApplicationSetup.FILE_SEPARATOR +
					indexPrefix +
					ApplicationSetup.LEXICONSUFFIX));
			}*/
			...
			final String path, final String prefix) throws IOException
	{
		...
		while (lexicon.readNextEntry()!=-1) {
			...
			offsets[termid] = i * lexiconEntrySize;
			i++;
		}
		lexicon.close();
