...
public class UTFLexicon extends Lexicon {
	...
	public boolean findTerm(int _termId) {
		try {
			...
			if (lexiconOffset == 0) {
				...
				lexiconFile.read(bt, 0, ApplicationSetup.STRING_BYTE_LENGTH - StringTools.utf8_length(term));
	...
	public boolean findTerm(String _term) {
		...
		if (_term.compareTo(currentTerm) == 0) {
			try {
				lexiconFile.read(bt, 0, ApplicationSetup.STRING_BYTE_LENGTH- StringTools.utf8_length(currentTerm));
			...
			if (i == 0) {
				...
				lexiconFile.seek(
					i * lexiconEntryLength
						- lexiconEntryLength
						+ 2//two bytes for the string length written by writeUTF
...
//				   buffer[offset++]&0xff) << 8 | buffer[offset++]&0xff) << 8 | buffer[offset++]&0xff) << 8 | buffer[offset++]&0xff;
	...
	public boolean updateEntry(
		...
		try {
			...
			lexiconFile.seek(  //utf length, string max length, termid, tf 
