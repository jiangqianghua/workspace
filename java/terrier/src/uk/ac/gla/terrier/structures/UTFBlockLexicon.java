...
public class UTFBlockLexicon extends BlockLexicon {
	...
	public UTFBlockLexicon() {
		...
		try {
			numberOfLexiconEntries = lexiconFile.length() / UTFBlockLexicon.lexiconEntryLength;
	...
	public boolean findTerm(String _term) {
		...
		while (high-low>1) {
			...
			i = (long)(high+low)/2;
			try {
				lexiconFile.seek((i * UTFBlockLexicon.lexiconEntryLength));
		...
		if (high == numberOfLexiconEntries)
			return false;
		try {
			lexiconFile.seek((high * UTFBlockLexicon.lexiconEntryLength));
			//lexiconFile.read(buffer, 0, termLength);
			tmpTerm = lexiconFile.readUTF();
			lexiconFile.read(bt, 0, ApplicationSetup.STRING_BYTE_LENGTH - StringTools.utf8_length(term));
		} catch (IOException ioe) {
	...
	public boolean seekEntry(int i) {
		try {
			...
			if (i == 0) {
				...
				startOffset = 0;
				startBitOffset = 0;
				//lexiconFile.read(buffer, 0, ApplicationSetup.STRING_BYTE_LENGTH);
				...
				lexiconFile.seek(i * lexiconEntryLength - lexiconEntryLength
						+ 2 + ApplicationSetup.STRING_BYTE_LENGTH + 12);
				startOffset = lexiconFile.readLong();
				startBitOffset = lexiconFile.readByte();
				startBitOffset++;
				if (startBitOffset == 8) {
