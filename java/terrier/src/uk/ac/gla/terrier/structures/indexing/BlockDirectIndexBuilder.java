...
public class BlockDirectIndexBuilder extends DirectIndexBuilder {
	 ...
	 * @deprecated
	 */
	public FilePosition addDocument(FieldDocumentTreeNode[] terms) throws IOException
	{
	 ...
	 * @since 1.1.0
	 */
	protected void addFieldDocument(FieldDocumentTreeNode[] fieldTerms) throws IOException {
		...
		int termCode = treeNode1.termCode;
		file.writeGamma(termCode +1);
		file.writeUnary(treeNode1.frequency);
		file.writeBinary(fieldTags, treeNode1.getFieldScore());
	
		int[] BlockIds = treeNode1.getBlockIds();
		int BlocksCount = BlockIds.length;
		file.writeUnary(BlocksCount + 1);
		if (BlocksCount > 0)
		{	
			file.writeGamma(BlockIds[0]+1);
			for (int i=1; i<BlocksCount; i++) {
				file.writeGamma(BlockIds[i] - BlockIds[i-1]);
			}
		}
		...
		for(int TermNo = 1; TermNo < termsCount; TermNo++) {
			...
			termCode = treeNode1.termCode;
			file.writeGamma(termCode - prevTermCode);
			file.writeUnary(treeNode1.frequency);
			file.writeBinary(fieldTags, treeNode1.getFieldScore());
	
			
			BlockIds = treeNode1.getBlockIds();
	 ...
	 *		 the document.
	 */
	protected void addNoFieldDocument(FieldDocumentTreeNode[] fieldTerms) throws IOException{
		BlockFieldDocumentTreeNode[] terms = 
			(BlockFieldDocumentTreeNode[])fieldTerms;
		final int termsCount = terms.length;
