...
public class TRECQrelsInMemory{
	 ...
	 * given query.
	 */
	public THashSet getNonRelevantDocuments(String queryid){
		THashSet relDocnos = null;
		for (int i = 0; i < this.getNumberOfQueries(); i++){
			if (qrelsPerQuery[i].queryid.equalsIgnoreCase(queryid)){
				relDocnos = (THashSet)qrelsPerQuery[i].nonRelDocnos.clone();
			}
		}
		return relDocnos;
	}
	 ...
	 * given query.
	 */
	public THashSet getRelevantDocuments(String queryid){
		THashSet relDocnos = null;
		for (int i = 0; i < this.getNumberOfQueries(); i++){
			if (qrelsPerQuery[i].queryid.equalsIgnoreCase(queryid)){
	 ...
	 * given query.
	 */
	public THashSet getRelevantDocuments(String queryid, int grade){
		THashSet relDocnos = null;
		for (int i = 0; i < this.getNumberOfQueries(); i++){
			if (qrelsPerQuery[i].queryid.equalsIgnoreCase(queryid)){
    ...
    public class QrelsHashSet{
            ...
            public QrelsHashSet(String queryid){
                    ...
                    nonRelDocnos = new THashSet();
                    relGrade = new TIntHashSet();
                    relGradeDocnosMap = new TIntObjectHashMap();
            }
             ...
             */
            public Object clone(){
                    QrelsHashSet dup = new QrelsHashSet(queryid);
                    dup.nonRelDocnos = (THashSet)nonRelDocnos.clone();
                    dup.relGrade = (TIntHashSet)relGrade.clone();
                    dup.relGradeDocnosMap = (TIntObjectHashMap)relGradeDocnosMap.clone();
                    return dup;
            }
            /**
            ...
            public boolean isRelevant(String docno){
            	...
            	for (int i=0; i<numOfGrades; i++){
            		if (((THashSet)relGradeDocnosMap.get(grades[i])).contains(docno)){
            			return true;
            		}
            	}
            	return false;
            ...
            public String[] getAllRelevantDocumentsToArray(){
            	...
            	for (int i=0; i<relGrade.size(); i++){
            		THashSet docnosTmp = (THashSet)relGradeDocnosMap.get(grades[i]);
             ...
             * @return The identifiers of all relevant documents.
             */
            public THashSet getAllRelevantDocuments(){
            	THashSet docnos = new THashSet();
            	int[] grades = relGrade.toArray();
            	for (int i=0; i<relGrade.size(); i++){
            		THashSet docnosTmp = (THashSet)relGradeDocnosMap.get(grades[i]);
            ...
            public String[] getRelevantDocumentsToArray(int grade){
            	if (!relGrade.contains(grade))
            		return null;
            	THashSet docnos = (THashSet)relGradeDocnosMap.get(grade);
            	return (String[])docnos.toArray(new String[docnos.size()]);
            }
            /**
             * Get the relevant documents for a given relevance grade.
             ...
             * grade.
             */
            public THashSet getRelevantDocuments(int grade){
            	if (!relGrade.contains(grade))
            		return null;
            	return (THashSet)relGradeDocnosMap.get(grade);
            }
            ...
            public void insertRelDocno(String docno, int grade){
            	if (!relGrade.contains(grade)){
            		...
            		((THashSet)relGradeDocnosMap.get(grade)).add(docno);
            	}
            }
