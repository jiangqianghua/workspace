...
public class TRECUTFCollection extends TRECCollection
{
		...
		public String getNextTerm() {
			...
			while (btag && ch != -1 && !EOD) {
				...
				try {
					...
					logger.fatal(
						"Input/Output exception during reading tokens. Stack trace follows", ioe);
	...
	{
		//return new TRECUTFDocument(new BufferedReader(new InputStreamReader(new LookAheadStream(br, end_docTag), desiredEncoding)), DocProperties);
		try{
            return new TRECUTFDocument(new BufferedReader(new InputStreamReader( new LookAheadStream(br, end_docTag), desiredEncoding)), DocProperties);
        } catch (java.io.UnsupportedEncodingException uee) {
            logger.warn("Desired encoding ("+desiredEncoding+") unsupported. Resorting to platform default.", uee);
            return new TRECUTFDocument(new BufferedReader(new InputStreamReader( new LookAheadStream(br, end_docTag))), DocProperties );
        }
	}
	...
	public Document getDocument(TagSet _tags, TagSet _exact, TagSet _fields) {
		//return new TRECUTFDocument(new BufferedReader(new InputStreamReader(new LookAheadStream(br, end_docTag), desiredEncoding)), DocProperties, _tags,_exact, _fields);
		try{
            return new TRECUTFDocument(new BufferedReader(new InputStreamReader( new LookAheadStream(br, end_docTag), desiredEncoding)), DocProperties, _tags,_exact, _fields);
        } catch (java.io.UnsupportedEncodingException uee) {
            logger.warn("Desired encoding ("+desiredEncoding+") unsupported. Resorting to platform default.", uee);
            return new TRECUTFDocument(new BufferedReader(new InputStreamReader( new LookAheadStream(br, end_docTag))), DocProperties, _tags,_exact, _fields);
        }
	}
