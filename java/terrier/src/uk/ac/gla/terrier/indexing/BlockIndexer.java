...
public class BlockIndexer extends Indexer {
	...
	protected final int BLOCK_SIZE = ApplicationSetup.BLOCK_SIZE;
	/** 
	...
	protected final int MAX_BLOCKS = ApplicationSetup.MAX_BLOCKS;
...
//			);
	...
	/**
	 * Adds a document to the direct and document indexes, as well as it's terms to the
	 * lexicon. This is called by indexDocument(), for collections where no field support
	 * is required.
	 * @param docid String the document identifier.
	 * @param termsInDocument FieldDocumentTree the terms in the document.
	 * @param numOfTokensInDocument int the number of indexed tokens in the document.
	 */
	protected void indexNoFieldDocument(String docid,
			int numOfTokensInDocument, 
			FieldDocumentTree termsInDocument) 
	{
		BlockFieldDocumentTreeNode[] termBuffer = 
			(BlockFieldDocumentTreeNode[]) termsInDocument.toArray();
		int TermsCount = termBuffer.length;
		try {
			...
			docIndexBuilder.addEntryToBuffer(docid, numOfTokensInDocument,
					dirIndexPost);
	...
	/**
	 * Adds a document to the direct and document indexes, as well as it's terms to the
	 * lexicon. This is called by indexDocument(), for collections where field support
	 * is required.
	 * @param docid String the document identifier.
	 * @param termsInDocument FieldDocumentTree the terms in the document.
	 * @param numOfTokensInDocument int the number of indexed tokens in the document.
	 */
	protected void indexFieldDocument(String docid, 
			int numOfTokensInDocument,
			FieldDocumentTree termsInDocument)
	{
		BlockFieldDocumentTreeNode[] termBuffer = (BlockFieldDocumentTreeNode[])termsInDocument.toArray();
		int TermsCount = termBuffer.length;
		try {
			/* add words to lexicontree */
			lexiconBuilder.addDocumentTerms(termBuffer);
	 ...
	 * @param numOfTokensInDocument int the number of indexed tokens in the document.
	 */
	protected void indexDocument(String docid, int numOfTokensInDocument,
			FieldDocumentTree termsInDocument) {
		if (useFieldInformation)
			indexFieldDocument(docid, numOfTokensInDocument, termsInDocument);
		else
			indexNoFieldDocument(docid, numOfTokensInDocument, termsInDocument);
	}
	/** 
	 * This adds a document to the direct and document indexes, as well 
	 * as it's terms to the lexicon. Handled internally by the methods 
