...
public class BasicIndexer extends Indexer
{
	...
	/** 
	 * Adds an entry to the direct index for a document 
	 * identified by docid. It also adds these terms to 
	 * the current LexiconTree for later writing to disk. 
	 * @param docid String the document identifier
	 * @param numOfTokensInDocument int the number of indexed tokens.
	 * @param termsInDocument FieldDocumentTree a structure that holds 
	 *		the terms that appear in a document.
	 */
	protected void indexNoFieldDocument(String docid, int numOfTokensInDocument, FieldDocumentTree termsInDocument){
		final FieldDocumentTreeNode[] termBuffer = termsInDocument.toArray();
	...
	{
		FieldDocumentTreeNode[] termBuffer = termsInDocument.toArray();
