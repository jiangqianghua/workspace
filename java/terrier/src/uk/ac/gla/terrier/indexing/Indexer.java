...
public abstract class Indexer
{
	...
	protected int MAX_DOCS_PER_BUILDER = 
		Integer.parseInt(ApplicationSetup.getProperty("indexing.max.docs.per.builder", "18000000"));
	 ...
	 */
	protected static boolean useFieldInformation = FieldScore.USE_FIELD_INFORMATION;
	...
	public void index(Collection[] collections) {
		...
		{
			String src = path + ApplicationSetup.FILE_SEPARATOR + prefix;
			String dest = path + ApplicationSetup.FILE_SEPARATOR + oldIndexPrefix;
			(new File(src+ApplicationSetup.PROPERTIES_SUFFIX)).renameTo(new File(dest+ApplicationSetup.PROPERTIES_SUFFIX));
			(new File(src+ApplicationSetup.IFSUFFIX)).renameTo(new File(dest+ApplicationSetup.IFSUFFIX));
			(new File(src+ApplicationSetup.DF_SUFFIX)).renameTo(new File(dest+ApplicationSetup.DF_SUFFIX));
			(new File(src+ApplicationSetup.LEXICON_INDEX_SUFFIX)).renameTo(new File(dest+ApplicationSetup.LEXICON_INDEX_SUFFIX));
			(new File(src+ApplicationSetup.LEXICONSUFFIX)).renameTo(new File(dest+ApplicationSetup.LEXICONSUFFIX));
			(new File(src+ApplicationSetup.DOC_INDEX_SUFFIX)).renameTo(new File(dest+ApplicationSetup.DOC_INDEX_SUFFIX));
			(new File(src+ApplicationSetup.LEXICON_HASH_SUFFIX)).renameTo(new File(dest+ApplicationSetup.LEXICON_HASH_SUFFIX));
		}
		//restore the prefix
	...
	protected static void mergeTwoIndices(String[] index1, String[] index2, String[] outputIndex){
		...
		//delete old files
		(new File(index1[0]+separator+index1[1]+ApplicationSetup.IFSUFFIX)).delete();
		(new File(index1[0]+separator+index1[1]+ApplicationSetup.DF_SUFFIX)).delete();
		(new File(index1[0]+separator+index1[1]+ApplicationSetup.LEXICON_INDEX_SUFFIX)).delete();
		(new File(index1[0]+separator+index1[1]+ApplicationSetup.LEXICONSUFFIX)).delete();
		(new File(index1[0]+separator+index1[1]+ApplicationSetup.DOC_INDEX_SUFFIX)).delete();
		(new File(index1[0]+separator+index1[1]+ApplicationSetup.PROPERTIES_SUFFIX)).delete();
		(new File(index1[0]+separator+index1[1]+ApplicationSetup.LEXICON_HASH_SUFFIX)).delete();
		
		(new File(index2[0]+separator+index2[1]+ApplicationSetup.IFSUFFIX)).delete();
		(new File(index2[0]+separator+index2[1]+ApplicationSetup.DF_SUFFIX)).delete();
		(new File(index2[0]+separator+index2[1]+ApplicationSetup.LEXICON_INDEX_SUFFIX)).delete();
		(new File(index2[0]+separator+index2[1]+ApplicationSetup.LEXICONSUFFIX)).delete();
		(new File(index2[0]+separator+index2[1]+ApplicationSetup.DOC_INDEX_SUFFIX)).delete();
		(new File(index2[0]+separator+index2[1]+ApplicationSetup.PROPERTIES_SUFFIX)).delete();
		(new File(index2[0]+separator+index2[1]+ApplicationSetup.LEXICON_HASH_SUFFIX)).delete();
	}
	...
	public static void merge(String mpath, String mprefix, LinkedList<String[]> llist, int counterMerged)
	{
		while (llist.size() > 1) {
			...
			llist = (LinkedList<String[]>)tmpList.clone();
