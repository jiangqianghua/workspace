...
public class TRECDocument implements Document {
	 ...
	 */
	public TRECDocument(Reader docReader, Map docProperties)
	{
		this.br = docReader;
	 ...
	 */
	public TRECDocument(Reader docReader, Map docProperties, TagSet _tags, TagSet _exact, TagSet _fields)
	{
		this.br = docReader;
		properties = docProperties;
		this._tags = _tags;
	...
	public static void dumpDocument(final Document d)
	{
		...
		while(! d.endOfDocument() )
		{
			...
			java.util.Iterator f = fields.iterator();
			if (f.hasNext())
				System.out.print((String)(f.next()));
			while(f.hasNext())
			{
				System.out.print(","+(String)(f.next()));
			}
