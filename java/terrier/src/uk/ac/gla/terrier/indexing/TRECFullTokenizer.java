...
/**
 * This class is the tokenizer used for indexing TREC collections. It can be
 * used for tokenizing other types of documents, provided that the tags to skip
...
public class TRECFullTokenizer implements Tokenizer {
	...
	protected String check(String s) {
		...
		try {
			...
			logger.fatal
					("Input / Output exception during checking a term. "
	...
	public void close() {
		try {
			...
			logger.fatal("Error while closing the buffered reader in the "+
					"TRECTokenizer class. Stack trace follows.", ioe);
			logger.fatal("Exiting ...");
			System.exit(1);
		}
	}
	...
	public void closeBufferedReader() {
		try {
			...
			logger.fatal("Error while closing the buffered reader in"+
			"the TRECTokenizer class. Stack trace follows.", ioe);
			logger.fatal("Exiting ...");
			System.exit(1);
		}
	}
	...
	public String nextToken() {
		...
		while (btag && ch != -1 & !EOD) {
			...
			try {
				...
				logger
						.fatal("Input/Output exception during reading tokens. Stack trace follows", ioe);
				logger.fatal("Exiting ...");
				System.exit(1);
			}
		}
