...
public class PhraseScoreModifier implements DocumentScoreModifier {
	...
	public Object clone() {
		return (Object)new PhraseScoreModifier((ArrayList)phraseTerms.clone(), required, blockDistance);
	}
	...
	public boolean modifyScores(Index index, MatchingQueryTerms terms, ResultSet set) {
		...
		final InvertedIndex inv = index.getInvertedIndex();
		if (! (inv instanceof BlockInvertedIndex))
		{
			logger.warn("Cannot perform phrase query, as the inverted index isn't a block index. Re-index with block.indexing=true property set.");
			return false;
		}
